...
export default function UserProfileScreen() {
  ...
  const numColumns = 3;
  const gap = 2;
  const containerWidth = centerContent ? Math.min(width, CONTENT_MAX_WIDTH) : width;
  const photoSize = (containerWidth - gap * (numColumns - 1)) / numColumns;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const loadData = async () => {
        ...
        // Load user's photos
        const photosResponse = await photoService.getUserPhotos(userId);
